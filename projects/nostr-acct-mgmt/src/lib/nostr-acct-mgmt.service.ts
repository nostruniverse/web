...
export class NostrAcctMgmtServiceConfiguration {

  browerSync: boolean;

  constructor() {
    this.browerSync = false;
  }
}
...
export class NostrAcctMgmtService {
  ...
  async isBrowserExtensionRunning(): Promise<boolean> {
    const res = await this.sendRequestToExtension(chrome.runtime.id, "nw-extension-hello", null);
    return res.success;
  }

  async sendAcctStorageToExtension(storage: AccountStorage | null) {
    const res = await this.sendRequestToExtension(chrome.runtime.id, "nw-extension-sync-acct-storage", {
      storage
    });
    return res.success;
  }
  ...
  async sendAcctSelectedToExtension(account: Account | null) {
    const res = await this.sendRequestToExtension(chrome.runtime.id, "nw-extension-sync-acct-selected", {
      account
    });
    return res.success;
  }

  async getAcctStorageFromExtension(): Promise<AccountStorage | null> {
    const res = await this.sendRequestToExtension(chrome.runtime.id, "nw-extension-get-acct-storage");
    return res.data;
  }

  async getSelectedAcctFromExtension(): Promise<Account | null> {
    const res = await  this.sendRequestToExtension(chrome.runtime.id, "nw-extension-get-selected-acct");
    return res.data;
  }
